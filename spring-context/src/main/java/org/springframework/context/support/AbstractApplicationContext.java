...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//执行自定义和spring内部定义的beanfactoryprocessor
				invokeBeanFactoryPostProcessors(beanFactory);

				//以上步骤为beanfanctory的初始化操作
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//对象与string类型转换 <property name="dao" ref="IndexDao"/>把indexDao转化成对象
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		//添加后置管理器，//用于处理实现ApplicationContextAware接口的类
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//getBeanFactoryPostProcessors在初次加载容器的时候获取到的为空，
		// 因为他是从AbstractBeanFactory.beanPostProcessors获取的
		//而beanPostProcessors只在AbstractApplicationContext.prepareBeanFactory添加了一个ApplicationContextAwareProcessor
		//
